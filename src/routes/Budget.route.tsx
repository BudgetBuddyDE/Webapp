...
const BudgetRoute = () => {
  ...
  const pageStats = React.useMemo(() => {
    const income = Array.from(subscriptionCategorySum.income).reduce((prev, [_category, amount]) => prev + amount, 0);
              ...
              <Paper elevation={0} sx={{ mt: '1rem' }}>
                {budgetTransactions.selected && (
                  <Box sx={{ ml: 2, mt: 1 }}>
                    ...
                    <Typography variant="subtitle1">{formatBalance(budgetTransactions.selected.amount)}</Typography>
                  </Box>
                )}
                ...
                <ParentSize>
                  {({ width }) => (
                    <BarChart
                      width={width}
                      height={width * 0.6}
                      data={budgetTransactions[chart === 'INCOME' ? 'income' : 'spendings'].daily.map((day) => ({
