...
import { MockedTransactions } from '@/__tests__/data';
...
describe('Validate earnings', () => {
  ...
  test('Earnings @ 01.08', () => {
    ...
    const result = TransactionService.calculateReceivedEarnings(MockedTransactions);
...
describe('Validate upcoming earnings', () => {
  ...
  test('Upcoming earnings @ 01.08', () => {
    jest.setSystemTime(new Date(2023, 7, 1));
    expect(new Date().getDate()).toBe(1);
    const result = TransactionService.calculateUpcomingEarnings(MockedTransactions);
    expect(result).toBe(1662.52);
  });
