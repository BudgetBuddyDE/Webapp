...
export const BudgetProgress: React.FC<TBudgetProgressProps> = ({category, amount_spent, budget}) => {
  ...
  return (
    <Box
      ...
      <Box sx={{display: 'flex', flex: 1, mr: 1}}>
        <Icon
          icon={budgetProgress === 100 ? <WarningRounded /> : hasBudgetExceeded ? <StopRounded /> : <SavingsRounded />}
...
export const BudgetProgressWrapper: React.FC<TBudgetProgressWrapperProps> = ({data, cardProps}) => {
  return (
    <Card {...cardProps}>
      <Card.Header>
        <Box>
      ...
      <Card.Body sx={{mt: 1}}>
        {data.length > 0 ? data.map(entry => <BudgetProgress key={entry.id} {...entry} />) : <NoResults />}
      </Card.Body>
    </Card>
  );
};
