...
import {Box, Grid, Paper, Skeleton, TextField, ToggleButton, ToggleButtonGroup, Typography} from '@mui/material';
...
export const DailyTransactionChart: React.FC<TDailyTransactionChart> = () => {
  ...
  const [chartContent, setChartContent] = React.useState<EDailyTransactionType>(EDailyTransactionType.SPENDINGS);
  const [dateRange, setDateRange] = React.useState({
    startDate: subDays(new Date(), 30),
    endDate: new Date(),
  });
  const [selectedTransaction, setSelectedTransaction] = React.useState<TDailyTransaction | null>(null);
  ...
  const handler: IDailyTransactionChartHandler = {
    ...
    onHoverAboveDailyTransaction(dailyTransaction) {
      if (!dailyTransaction) {
        setSelectedTransaction(selectedData[selectedData.length - 1] || {date: new Date(), amount: 0});
        return;
      }
  ...
  React.useEffect(() => {
    if (!session) return;
    setLoading(true);
    TransactionService.getDailyTransactions(dateRange.startDate, dateRange.endDate, chartContent, authOptions)
        ...
        <Paper elevation={0} sx={{mt: '1rem'}}>
          {!loading && selectedTransaction && (
            <Box sx={{ml: 2, mt: 1}}>
              ...
              <Typography variant="subtitle1">{formatBalance(Math.abs(selectedTransaction.amount))}</Typography>
            </Box>
          )}
