...
export interface IDailyTransactionStore {
  ...
  fetchedAt: Date | null;
  setFetchedData: (key: EDailyTransactionType, data: TDailyTransaction[], fetchedBy: TUser['uuid'] | null) => void;
  clear: () => void;
}
...
export const useDailyTransactionStore = create<IDailyTransactionStore>(set => ({
  ...
  fetchedBy: null,
  fetchedAt: null,
  setFetchedData: (key, data, fetchedBy) => set({[key]: data, fetchedBy: fetchedBy, fetchedAt: new Date()}),
