import { type PaymentMethodInputOption, applyPaymentMethodOptionsFilter } from './PaymentMethodAutocomplete.component';
...
describe('Validate if correct items are returned by filter', () => {
    const filterOptionsState = (keyword: string) => ({
        inputValue: keyword,
        getOptionLabel(option: PaymentMethodInputOption) {
            return option.label;
        },
    });
    const options: PaymentMethodInputOption[] = [
        { label: 'Cash', value: 1 },
