import { type CategoryInputOption, applyCategoryOptionsFilter } from './CategoryAutocomplete.component';
...
describe('Validate if correct items are returned by filter', () => {
  const filterOptionsState = (keyword: string) => ({
    inputValue: keyword,
    getOptionLabel(option: CategoryInputOption) {
      return option.label;
    },
  });
  const options: CategoryInputOption[] = [
    { label: 'Work', value: 1 },
    { label: 'Rent', value: 2 },
    { label: 'Other', value: 3 },
