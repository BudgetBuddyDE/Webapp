...
export const NoResults: React.FC<INoResultsProps> = ({ sx, text = 'No items found' }) => {
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }}>
                ...
                <Chip
                  ...
                  onDelete={() => {
                    setFilter((prev) => {
                      ...
                      const filteredCategories = (categories as number[]).filter((id) => id !== matchedCategory.id);
                      return { ...prev, categories: filteredCategories.length === 0 ? null : filteredCategories };
                    });
                  }}
                  size="small"
        ...
        {filter.paymentMethods
          ? filter.paymentMethods.map((paymentMethodId) => {
              const matchedPaymentMethod = fetchPaymentMethods.paymentMethods.find(({ id }) => id === paymentMethodId);
                ...
                <Chip
                  ...
                  onDelete={() => {
                    setFilter((prev) => {
                      ...
                      return {
                        ...prev,
                        paymentMethods: filteredPaymentMethods.length === 0 ? null : filteredPaymentMethods,
                      };
                    });
                  }}
                  size="small"
